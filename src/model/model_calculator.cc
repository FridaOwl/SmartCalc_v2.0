...
bool MyCalculator::ModelCalculator::isFunction(char c) { return isalpha(c); }

void MyCalculator::ModelCalculator::processOperator(
    char op, std::stack<double> &operands) {
  if (operands.size() < 2) {
...
void MyCalculator::ModelCalculator::processFunction(
    char func, std::stack<double> &operands) {
  if (operands.empty()) {
...
double MyCalculator::ModelCalculator::unaryCalculate(char func,
  ...
  switch (func) {
    ...
    case 'n':
      return atan(operand);
    case 'q':
      return sqrtCheck(operand);
    case 'l':
      return logCheck(operand);
    case 'g':
      return log10Check(operand);
    case 'u':
      return -operand;
    default:
...
double MyCalculator::ModelCalculator::binaryCalculate(char operation,
  ...
  switch (operation) {
    ...
    case '*':
      return operand1 * operand2;
    case '/':
      return divisionCheck(operand1, operand2);
    case '%':
...
double MyCalculator::ModelCalculator::divisionCheck(double operand1,
                                                    double operand2) {
  if (operand2 == 0.0) {
